...
import (
	...
	"github.com/albanybuipe96/bookstore-users-api/data/postgres/datasource"
	"github.com/albanybuipe96/bookstore-users-api/data/postgres/queries"
...
func (user *User) Save() *errors.CustomError {
	...
	err := datasource.DbClient.QueryRow(
		query.Insert(),
		user.FirstName,
		user.LastName,
		user.Email,
...
func (user *User) Update() *errors.CustomError {
	query := queries.Query{TableName: "users", DbEngine: queries.PostgreSQL}
	_, err := datasource.DbClient.Exec(
		query.Update(),
		user.FirstName,
		user.LastName,
		user.Email,
...
func (user *User) GetAllUsers() ([]*User, *errors.CustomError) {
	query := queries.Query{TableName: "users"}
...
func (user *User) Delete() *errors.CustomError {
	query := queries.Query{TableName: "users", DbEngine: queries.PostgreSQL}
	_, err := datasource.DbClient.Exec(query.Delete(), user.Id)
	if err != nil {
		log.Println(err.Error())
		return errors.ReportDbError(err)
	}
